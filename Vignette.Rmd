...
```{r eval=TRUE, echo=T}
GSEA.result <- GSEA_run(Arranged.Genes$Gene,chip_index,get.RES = T,RES.filter = c("EPAS1","HIF1A","ARNT","MXI1"))
```


  v) Plotting the results.
TFEA.ChIP includes three functions to plot your GSEA results using the package *plotly*. With the tree functions provided, *plot_ES*, *plot_all_RES*, and *plot_some_RES*, the user can generate interactive html plots to visualize their results with detail.

v.i) Plotting Enrichment Scores with *plot_ES*. We can choose to highlight ChIP-Seq from specific transcription factors plotting them in a particular color.

```{r eval=FALSE, echo=T}
TF.hightligh<-c("EPAS1","HIF1A","ARNT")
names(TF.hightligh)<-c("EPAS1","HIF1A","ARNT")
col<- c("red","blue","green")
plot_ES(GSEA.result,LFC = Arranged.Genes$log2FoldChange,specialTF = TF.hightligh,TF_colors = col)
```


v.ii) Plot Runing Enrichment Scores with *plot_all_RES*. This function will plot **all** the RES stored in the *GSEA.run* output. It is only recommended to use if the user has previously restricted the RES stored in *GSEA.run* with the variable *RES.filter*
    
```{r eval=FALSE, echo=T}
plot_all_RES(GSEA_result = GSEA.result,LFC = Arranged.Genes$log2FoldChange)
```



v.iii) Plot a subset of the Running Scores with *plot_some_RES*. This funtion allows to plot a given subset of the Running Enrichment Scores stored. It's useful to plot only the RES corresponding to a particular TF, or to discard the RES of those ChIP-Seq that did not get a significant p-value.
...
```
  ...
  iv) Load the Dnase Hypersensitive Sites database and generate your TFBS database with *GR2tfbs_db*
...
```
  ...
  v) Generate a binary matrix to use with the rest of TFEA.ChIP using *makeTFBSmatrix*. This matrix and the metadata table are the files you will need to run TFEA.ChIP with your own data.
...
```
  ...
  vi) At the beginning of a session, use the function *set_user_data* to use your TFBS binary matrix and metadata table with the rest of the package.
