...
plot_CM(pval_mat_UP)
```
In fact, the exploration of this graph shows a strong enrrichment for several HIF datasets, as expected.
...
```{r eval=TRUE, echo=T}
GSEA.result <- GSEA.run(Arranged.Genes$Gene,get.RES = T,RES.filter = c("EPAS1","HIF1A","ARNT","MXI1"))
```
...
```{r eval=TRUE, echo=T,fig.width=7, fig.height=4}
