...
To filter the peaks of the diferent experiments we use a database of Dnase Hypersensitive Sites. Specifically, the ChIPseq peaks in each dataset are filtered to select those overlapping or near to (by default max. distance 10 nucleotides) DNase HS regions located within 1Kb of genes. 
...
This code should only be executed to generate a new GRanges object containing DNAse HS sites

5. Assign TFBS peaks from ChIP dataset to specific genes
...
txdb<-TxDb.Hsapiens.UCSC.hg19.knownGene
Genes<-genes(txdb)
TF.gene.binding.db<-GR2tfbs_db(Genes,gr.list) # gr.list is a list of outputs from the funcion "txt2GR"
```
