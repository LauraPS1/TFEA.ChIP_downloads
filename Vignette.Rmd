...
if("ggplot2" %in% rownames(installed.packages()) == TRUE){ 
    ...
    ggplot(pval_mat_UP,aes(x=log.adj.pVal, y=OR))+
        geom_point(aes(col=highlight))+
        labs(title="Transcription factor enrichment", subtitle="HUVEC cells under hypoxia",
        ...
        theme_bw()+
        scale_color_manual(values = c("Other"="lightsteelblue3"))+
...
If your dataset contains genes with a log(Fold Change) of Inf or -Inf, we recommend to filter those that do not haver a significant p-value out of the list.
...
if("ggplot2" %in% rownames(installed.packages()) == TRUE){ 
    ...
    ggplot(table,aes(x=Arg.ES, y=ES))+
        geom_point(aes(col=highlight))+
        labs(title="Transcription factor enrichment", subtitle="HUVEC cells under hypoxia",
...
if("ggplot2" %in% rownames(installed.packages()) == TRUE){ 
    ...
    table %>%
        ...
        ggplot(aes(x=argument, y=value, colour=Accesion)) +
        geom_line()+
        labs(title="Running Enrichment Scores", subtitle="HUVEC cells under hypoxia",
             y="RES", x="Argument",
             caption="To limit this file's size, only static versions of the plots are shown in this vignette")+
        theme_bw()+
        geom_hline(yintercept = 0)
